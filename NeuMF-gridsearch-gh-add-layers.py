...
from evaluateGH import evaluate_model
from DatasetGH import Dataset
...
def parse_args():
    ...
    parser.add_argument('--dataset', nargs='?', default='gharchive-5k',
...
def get_model(num_users, num_items, activity_items, mf_dim=10, layers=[10], reg_layers=[0], reg_mf=0):
    assert len(layers) == len(reg_layers)
    num_layer = len(layers)  # Number of layers in the MLP
    # Input variables
    user_input = Input(shape=(1,), dtype='int32', name='user_input')
    item_input = Input(shape=(1,), dtype='int32', name='item_input')
    activity_input = Input(shape=(1,), dtype='int32', name='activity_input')
    ...
    MLP_Embedding_User = keras.layers.Embedding(input_dim=num_users, output_dim=int(layers[0] / 3),
                                                name="mlp_embedding_user",
                                                embeddings_initializer=keras.initializers.random_normal(mean=0.0,
                                                                                                        stddev=0.01),
                                                embeddings_regularizer=regularizers.l2(reg_layers[0]))
    MLP_Embedding_Item = keras.layers.Embedding(input_dim=num_items, output_dim=int(layers[0] / 3),
                                                name='mlp_embedding_item',
                                                embeddings_initializer=keras.initializers.random_normal(mean=0.0,
                                                                                                        stddev=0.01),
                                                embeddings_regularizer=regularizers.l2(reg_layers[0]))

    MLP_Embedding_Activity = keras.layers.Embedding(input_dim=activity_items, output_dim=int(layers[0] / 3),
                                                name='mlp_embedding_activity',
                                                embeddings_initializer=keras.initializers.random_normal(mean=0.0,
                                                                                                        stddev=0.01),
                                                embeddings_regularizer=regularizers.l2(reg_layers[0]))
    ...
    mlp_actity_latent = keras.layers.Flatten()(MLP_Embedding_Activity(activity_input))
    mlp_vector = keras.layers.Concatenate()([mlp_user_latent, mlp_item_latent, mlp_actity_latent])
    ...
    model = keras.Model(inputs=[user_input, item_input, activity_input],
                        outputs=prediction)
...
def get_train_instances(train, num_negatives, activity_List):
    user_input, item_input, labels, activity_input = [], [], [], []
    num_users = train.shape[0]
    for (u, i) in train.keys():
        # positive instance
        user_input.append(u)
        item_input.append(i)
        activity_input.append(activity_List[u])
        ...
        for t in range(num_negatives):
            ...
            activity_input.append(activity_List[u])
            labels.append(0)
    return user_input, item_input, labels, activity_input
...
if __name__ == '__main__':
    ...
    for idx, params in enumerate(param_combinations):
        ...
        train, testRatings, testNegatives, activity_List = dataset.trainMatrix, dataset.testRatings, dataset.testNegatives, dataset.activityList
        ...
        # Build model
        activity_items = 29449
        model = get_model(num_users, num_items, activity_items, mf_dim, layers, reg_layers, reg_mf)
        ...
        keras.utils.plot_model(
            model, to_file='chart/model-ex1.png', show_shapes=True, show_dtype=True,show_layer_activations=True,
        ...
        for epoch in range(num_epochs):
            ...
            user_input, item_input, labels, activity_input = get_train_instances(train, num_negatives, activity_List)

            # Training
            hist = model.fit([np.array(user_input), np.array(item_input),  np.array(activity_input)], # input
